...
#include <utad_car_msgs/SensorState.h>
#include <utad_car_msgs/VersionInfo.h>
#include <string>

#define SOFTWARE_VERSION "1.2.5"
#define HARDWARE_VERSION "2021.06.04"
#define FIRMWARE_VERSION_MAJOR_NUMBER 1
#define FIRMWARE_VERSION_MINOR_NUMBER 2

ros::Publisher car_version_info_pub;
ros::Publisher car_diagnostics_pub;
...
void sensorStateMsgCallback(const utad_car_msgs::SensorState::ConstPtr &msg)
{
  if (msg->battery > 11.0)
  ...
  if (msg->button == utad_car_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == utad_car_msgs::SensorState::BUTTON1)
...
void firmwareVersionMsgCallback(const utad_car_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  utad_car_msgs::VersionInfo version;

  version.software = SOFTWARE_VERSION;
  version.hardware = HARDWARE_VERSION;
  version.firmware = msg->firmware;

  car_version_info_pub.publish(version);
}

void msgPub()
{
  diagnostic_msgs::DiagnosticArray car_diagnostics;

  car_diagnostics.header.stamp = ros::Time::now();

  car_diagnostics.status.clear();
  car_diagnostics.status.push_back(imu_state);
  car_diagnostics.status.push_back(motor_state);
  car_diagnostics.status.push_back(LDS_state);
  car_diagnostics.status.push_back(battery_state);
  car_diagnostics.status.push_back(button_state);

  car_diagnostics_pub.publish(car_diagnostics);
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "utad_car_diagnostic");
  ros::NodeHandle nh;

  car_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
  car_version_info_pub = nh.advertise<utad_car_msgs::VersionInfo>("version_info", 10);

  ros::Subscriber imu         = nh.subscribe("imu", 10, imuMsgCallback);
  ros::Subscriber lds         = nh.subscribe("scan", 10, LDSMsgCallback);
  ros::Subscriber car_sensor  = nh.subscribe("sensor_state", 10, sensorStateMsgCallback);
